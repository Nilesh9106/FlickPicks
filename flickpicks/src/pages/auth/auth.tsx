...
import { postCall } from "../../components/api";
...
const Login = () => {
    ...
    const handleSubmit = async () => {
        setLoading(true)
        const data = await postCall("user/login", { username, password });
        setLoading(false)
...
const SignUp = () => {
    ...
    const handleSubmit = async () => {
        setLoading(true)
        const data = await postCall("user/register", { username, email, password });
        setLoading(false)
        if (data?.status == "success") {
            localStorage.setItem("token", data.token);
            localStorage.setItem("user", JSON.stringify(data.userInfo));
